...
class FP8_Optimizer(ZeROOptimizer):
    ...
    def __init__(self,
        ...
        assert grad_acc_dtype in [torch.float32, torch.bfloat16
                                  ], f"FP8_Optimizer: Unsupported gradient accumulation data type: {grad_acc_dtype}"
    ...
    def _setup_for_real_optimizer(self):
        ...
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            if param_group.get('group_dtypes'):
                ...
                self._setup_for_fp8_params(real_dp_world_size, i, param_group, partition_id, trainable_parameters)
            else:
                ...
                self._setup_for_bf16_params(real_dp_world_size, i, param_group, partition_id, trainable_parameters)
    ...
    def _setup_for_bf16_params(self, real_dp_world_size, i, param_group, partition_id, trainable_parameters):
        ...
        self.fp32_groups_flat_partition.append(bf16_dp_partitions[partition_id].clone().to(
            self.master_weights_dtype).detach())
    ...
    def _setup_for_fp8_params(self, real_dp_world_size, i, param_group, partition_id, trainable_parameters):
        ...
        for dtype in dtypes:
            ...
            # divide flat weights into equal sized partitions
            partition_size = self.fp8_param_groups_dict[dtype]["param_groups_flat"][i].numel() // real_dp_world_size
            fp8_dp_partitions = [
            ...
            if first_iter:
                # create fp32 params partition
                self.fp32_groups_flat_partition.append(fp8_dp_partitions[partition_id].clone().to(
                    self.master_weights_dtype).detach())
                self.fp32_groups_flat_partition[i].requires_grad = True
                ...
                # create fp32 gradients
                fp32_flat_buffer = torch.zeros_like(self.fp8_param_groups_dict[dtype]["param_groups_flat"][i],
    ...
    def _link_all_hp_params(self):
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            if param_group.get('group_dtypes'):
                ...
                partition_size = self.fp8_param_groups_dict[
                    dtypes[0]]["param_groups_flat"][i].numel() // real_dp_world_size
            ...
            for lp in group:
                if lp._hp_mapping is not None:
                    ...
                    for key in lp._hp_mapping.get_optim_state_keys():
                        opt_keys.add(key)
            map_to_flat_opt_states(param_group['params'][0], group, self.optimizer.state, opt_keys)
