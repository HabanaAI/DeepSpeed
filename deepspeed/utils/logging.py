...
class LoggerFactory:
    ...
    def create_logger(name=None, level=logging.INFO):
        ...
        # TODO SW-213123: verify that PT version is 2.6 or higher
        if os.getenv("DISABLE_LOGS_WHILE_COMPILING", "0") == "1":
            excluded_dict = set(os.getenv("LOGGER_METHODS_TO_EXCLUDE_FROM_DISABLE", "").split(","))
            ignore_dict = {'info', 'debug', 'error', 'warning', 'critical', 'exception', 'isEnabledFor'
                           } - excluded_dict
            for method in ignore_dict:
