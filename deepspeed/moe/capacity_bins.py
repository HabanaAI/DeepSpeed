...
class CapacityBins(torch.nn.Module):
    ...
    def _save_device(self, tensor):
        if self.device is None:
            ...
            self.model_device = tensor.device
    ...
    def _get_capacity_bins(self, gate_output: torch.Tensor) -> Union[torch.Tensor, None]:
        ...
        if self.min_tokens_per_expert is None:
            ...
            n_tokens_in_micro_batch = torch.tensor(gate_output.shape[0], device=gate_output.device).to(torch.long)
